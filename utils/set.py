...
def set_optimizer(args, backbone, classifier, session):
    if session ==0:
        ...
        elif args.train_parames == 'Head':
            for name, param in backbone.named_parameters():
                if 'layer1' in name :
                    param.requires_grad_(True)
                else:
                    param.requires_grad_(False)
            Head3_params = filter(lambda p: p.requires_grad, backbone.parameters())
            backbone_optimizer = optim.SGD(Head3_params, lr=args.lr, momentum=0.9, weight_decay=5e-4)
        
        elif args.train_parames == 'Bottom':
            for name, param in backbone.named_parameters():
                if 'layer3' in name :
                    param.requires_grad_(True)
                else:
